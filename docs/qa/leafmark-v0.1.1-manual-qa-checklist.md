...
- [x] swift test 通过，当前应为 45 tests / 0 failures。
- [x] swift build 输出 Build complete。
- [x] build/LeafMark.app 生成成功。
- [x] 两个 plist 都输出 OK。
...
- [x] Editor 显示欢迎 Markdown。
- [x] Preview 显示渲染后的欢迎文档。
- [x] Status area 显示保存状态、字数、字符数、路径状态。
- [x] 无崩溃、无空白窗口。
...
- [ ] 左侧显示文件夹目录树。
这里有问题啊，打不开目录树。打开一个目录的时候，Open的按钮是置灰的。
- [ ] .md、.markdown、.txt 显示。
- [ ] unsupported 文件，例如 .pdf，会显示在目录树中。
- [ ] unsupported 文件点击或双击后不打开、不报错。
- [ ] 文件夹在文件前面显示。
- [ ] 子文件夹可展开/折叠。
